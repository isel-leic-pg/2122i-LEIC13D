fun main(){
    ...
    println(mapWithMutableList(l1,{i -> i +2}))

    println(l1.filter({i -> i % 2 == 0}))
    println(filter(l1,{i -> i % 2 == 0}))
    println(filterWithMutableList(l1,{i -> i % 2 == 0}))
...
fun filterWithMutableList(l : List<Int>, pred : (Int) -> Boolean) : List<Int>{
    val result = mutableListOf<Int>()
    var i = 0
    while(  i < l.size){
        //result = if(pred(l[i])) result + l[i] else result
        if(pred(l[i])) result.add(l[i])
        ++i // i = i + 1
    }
    return result
}
...
fun mapWithMutableList(l : List<Int>, trans : (Int) -> Int) : List<Int>{
    val result = mutableListOf<Int>()
    var i = 0
    while(  i < l.size){
        //result = result + trans(l[i])
        result.add(trans(l[i]))
        ++i // i = i + 1
    }
    return result
}
